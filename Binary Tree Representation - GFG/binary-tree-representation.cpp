...
public:
    ...
    void create_tree(node* root, vector<int> &vec){
        //Your code goes here
        root->data = vec[0];
        node* leftChild = root->left = newNode(vec[1]);
        node* rightChild = root->right = newNode(vec[2]);
        leftChild->left = newNode(vec[3]);
        leftChild->right = newNode(vec[4]);
        rightChild->left = newNode(vec[5]);
        rightChild->right = newNode(vec[6]);
    }
